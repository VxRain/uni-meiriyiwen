...
export default class Request {
  ...
  async request(
    ...
    try {
      ...
      const _res = (this.responseInterceptor &&
        this.responseInterceptor(
          { isCancel: true, response: e },
          options
        )) || { isCancel: true, response: e };
      _callbackWrapper(_res, [
        this.fail,
        failHandler,
        this.complete,
        completeHandler,
    ...
    if (config.isReqToCancel) {
      const _res = (this.responseInterceptor &&
        this.responseInterceptor(
          { isCancel: true, response: config },
          options
        )) || { isCancel: true, response: config };
      _callbackWrapper(_res, [
        this.fail,
    ...
    if (task) {
      const cg = _configWrapper(
        null,
        null,
        config,
        options,
        this.responseInterceptor,
        [this.success, successHandler, this.complete, completeHandler],
        [this.fail, failHandler, this.complete, completeHandler]
      );
...
function _configWrapper(
  ...
  config["success"] = (response) => {
    ...
    if (_res && _res.isReqToReject) {
      delete _res.isReqToReject;
  ...
  config["fail"] = (response) => {
    let _res = response;
    if (interceptor) {
      _res = interceptor({ isFail: true, response: response }, options);
    }
    delete _res.isReqToReject;
    _callbackWrapper(_res, failFns);
    reject && reject(_res);
