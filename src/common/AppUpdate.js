/**
 * @file App检查更新弹窗，使用nativeObj.View实现
...
// 主颜色
// 默认颜色：#FF5B78，Material颜色：#3949AB
const $mainColor = "#3949AB";
// 弹窗图标url
const $iconUrl = "/static/common/update-rocket.png";
...
/**
 * 发起请求获取服务端版本号
 * @param {*} version 本地的版本信息
 * @param {*} isPrompt
 */
const getServerNo = function(version, isPrompt = false) {
  /**
   * @var {Object}
   * @desc 发送的请求对象
   * @property  {Number} type 应用的平台，1101是安卓，1102是IOS
   */
  let httpData = {
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve({
      ...
      versionInfo:
        "修复了一些问题:\n更新说明1...\n更新说明2...\n更新说明3...\n更新说明4...\n更新说明5...\n更新说明6...\nabcdefg",
      updateType: "solicit",
      downloadUrl: "http://web.mall.jiulianshop.com/app/res/app1.1.6.apk",
    }).then((res) => {
      /* res的数据说明
       * | 参数名称	     | 一定返回 	| 类型	    | 描述
       * | -------------|--------- | --------- | ------------- |
       ...
       * | downloadUrl	 | y	    | String	| 版本下载链接（IOS安装包更新请放跳转应用商店链接,安卓apk和wgt文件放文件下载链接）  |
       */
...
function updatePopup(data, callback) {
  ...
  const textHeight = 18; // 文本的高度
  let contentTop = 130; // 距离顶部的距离

  // 遍历更新说明文本列表，并push到弹窗的内容列表里，为绘制做准备
  ...
  popupView.animate(
    { type: "shrink", frames: 20, region: { top: "44px", bottom: "48px" } },
    function() {
...
/**
 * 检测App更新
 * @param {Boolean} isPrompt 没有新版本时是否显示提示
 */
export default function(isPrompt = false) {
  // 获取本地当前版本号
  getCurrentNo()
    .then((versionInfo) => getServerNo(versionInfo, isPrompt)) // 获取线上的当前版本号
    .then((res) => {
      // 根据返回的数据进行逻辑操作
